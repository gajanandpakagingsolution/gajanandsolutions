 ...
 */
const PlaceholderImage = ({ src, alt, label, refCode, className = "" }) => {
  ...
  if (resolvedSrc) {
    return (
      <img
        src={resolvedSrc}
        alt={alt || label}
        className={`${className} block rounded-2xl shadow-md`}
      />
    );
  }
  return (
    <div
      className={`${className} flex flex-col items-center justify-center gap-1 bg-gray-100 border-2 border-dashed border-gray-300 text-center px-2`}
    >
